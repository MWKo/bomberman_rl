...
def setup(self):
    ...
    if self.train or not os.path.isfile(MODEL_FILE_NAME):
        ...
        self.model = [RegressionForest(10) for _ in range(len(ACTIONS))]
        self.trained = False
    else:
...
def act(self, game_state: dict) -> str:
    ...
    q_vector = np.array([forest.predict(state_to_features(game_state)) for forest in self.model]) if self.trained else 0
    if self.train:
        self.q_vectors.append(q_vector)

    if (self.train and random.random() < random_prob) or not self.trained:
