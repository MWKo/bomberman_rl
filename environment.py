...
class GenericWorld:
    ...
    def add_agent(self, agent_dir, name, train=False, config_fname=None):
        assert len(self.agents) < s.MAX_AGENTS

        # if self.args.single_process:
        backend = SequentialAgentBackend(config_fname, train, name, agent_dir)
        # else:
...
class BombeRLeWorld(GenericWorld):
    ...
    def setup_agents(self, agents):
        ...
        self.agents = []
        for agent_dir, train, config_fname in agents:
            if list([d for d, _, _ in agents]).count(agent_dir) > 1:
                ...
                name = agent_dir
            self.add_agent(agent_dir, name, train=train, config_fname=config_fname)
